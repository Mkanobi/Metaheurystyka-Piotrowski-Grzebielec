...
def ant_walk_mult(weights: list, pheromones: list, starting_city: list, beta: int, rand: int, added: list, result: list) -> None:
    for str_ct in starting_city:
        ant_walk(weights, pheromones, str_ct, beta, rand, added, result)

# beta only control
# synonymus to alpha=1
def ant_walk(weights: list, pheromones: list, starting_city: int, beta: int, rand: int, added: list, result: list) -> None:
    dim = len(weights)
    path = [starting_city]
    path_len = 0
    ...
    for itr in range(dim-1):
        city = path[-1]
        ppb = [
            pheromones[city][k] * pow((1 / weights[city][k]), beta) if weights[city][k] != 0 else -1) if k not in path else 0.0 for k in range(dim)]
        if -1 not in ppb:
        ...
        choice = choices(range(dim), ppb)[0] if randrange(100)+1 <= rand else greedy_choice
...
def ant_colony(problem, constraint: int, colony_size: int, t_cnt: int, beta: int, rho: int, elite: int, rand: int) -> list:
    start = time.time()
    result = [[], math.inf]
    ...
    while time.time() - start < constraint:
        ...
        for starting_city in starting_points:
            threads.append(Thread(
                ...
                args=[weights, pheromones, starting_city, beta, rand, added, paths]))
