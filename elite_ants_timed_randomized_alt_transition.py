...
# Alpha only control
# Multiplicative, single argument, "part based"
# Alpha 0.2 will mean pheromone makes 20% of ppb, distance makes 80%
def ant_walk(weights: list, pheromones: list, starting_city: int, alpha: int, rand: int, added: list, result: list) -> None:
    ...
    for itr in range(dim-1):
        ...
        ppb = [
            ...
            (1 / weights[city][k]) * (1 - alpha)) if weights[city][k] != 0 else -1) if k not in path else 0.0 for k in range(dim)]
        if -1 not in ppb:
